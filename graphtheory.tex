...
\begin{lem}
Let $D' = D - K$ where $K =\outcut(X)$ as above. Then $D'$ contains no
directed $(x,y)$-walks.
\end{lem}
\begin{defn}
By definition, since $x \in X$ and $y \not\in X$, every directed walk
starting from $x$ and ending in $y$ starts out in $X$ and ends in $V_D
\setminus X$. In particular, along such a walk, we can consider the first
vertex $v$ arising in the walk which is in $V_D \setminus X$. The
preceeding vertex, $w$ is therefore in $X$ and the arrow between them in
the directed walk $a$ must therefore start at $w \in X$ and end in $v \in
V_D \setminus X$. But this means that $a \in \outcut(X)$, a contradiction.
\end{defn}
