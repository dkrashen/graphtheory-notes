...
walk}\index{walk!directed} $P$ in $D$ to be a sequence of vertices and
arrows
...
\begin{defn}
If $D$ is a digraph, we define an \textbf{undirected
walk}\index{walk!undirected} $P$ in $D$ to be a sequence of vertices and
arrows
\[P = v_1 a_2 v_2 a_3 \cdots a_n v_n \]
such that for each arrow $a_i$ we have
either
\begin{itemize}
\item $s(a_i) = v_{i-1}$ and $t(a_i) = v_i$ or
\item $t(a_i) = v_{i-1}$ and $s(a_i) = v_i$.
\end{itemize}
\end{defn}

We similarly define directed and undirected paths, trails and circuits in
an analogous
fashion.\index{path!directed}\index{path!undirected}\index{trail!undirected}\index{trail!directed}\index{circuit!directed}\index{circuit!undirected}

\begin{defn} \label{def:forward and backwards arrows}
Suppose that $P = v_1 a_2 \cdots a_n v_n$ is an undirected path in $D$. We
say that $a_i$ is a \Xb{forward arrow} in $P$ if $s(a_i) = v_{i-1}$ and
$t(a_i) = v_i$. We say that $a_i$ is a \Xb{backwards arrow} in $P$ if
$s(a_i) = v_i$ and $t(a_i) = v_{i-1}$.
\end{defn}
Note that being forward or backward is not an intrinsic property of an
arrow, but rather has to do with an arrow's relationship to a particular
path. We remark that this concept is more easily defined for paths rather
than general walks, since in a walk a given arrow may arise multiple times
in either orientation (although defining this for trails would be fine as
well).

\subsection{Arrow cuts}

\begin{defn}
Let $D$ be a digraph and $x, y \in V_D$. We say that $S \subset A_D$ is an
$(x,y)$-\Xb{arrow cut} if $D - S$ contains no directed $(x,y)$-paths. We say
that it is minimal if there is no $(x, y)$-arrow cut $S'$ with $S'$ a proper
subset of $S$.
\end{defn}

\begin{exercise} \label{exercise:min arrow cut}
Show that if $K$ is a minimal $(x,y)$-arrow cut, if and only if we can
partition the vertices of $V_D$ as $V_D = X \cup Y$ with $x \in X, y \in Y$
and $X \cap Y = \emptyset$, and with $K = [X, Y]$.
\end{exercise}
...
$P$. We say that $a$ is \textbf{unsaturated}\index{unsaturated (arrow in a
path in a network)} if $a$ is a forward arrow in $P$ with $f(a) < c(a)$, or
if $a$ is backwards arrow with $f(a) \neq 0$. We say that $a$ is
\textbf{saturated}\index{saturated (arrow in a path in a network)} if it is
not unsaturated.
\end{defn}
...
\textbf{increasing}\index{increasing path for a network flow} if every
